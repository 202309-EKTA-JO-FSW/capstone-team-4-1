...
export default function AddItem({
  ...
  useEffect(() => {
    ...
    fetch(`${urlService.serverUrl}/customer/dishes/${dishID}`, {
      headers: headers,
    })
      .then((res) => res.json())
  ...
  return (
    <div className="z-50 fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full addModal">
      <div
        className="relative mx-auto my-0 top-1/2 -right-1/2 left-0 -translate-y-1/2 border
      ...
      >
        <button
          onClick={handleClose}
          className="absolute top-5 right-5 p-1 rounded-full transition-all duration-200 ease-in-out hover:bg-gray-200
        ...
        >
          <svg
            ...
            className="text-black transform transition-transform duration-200 ease-in-out hover:rotate-45 hover:text-[#FFC245]
          xl:h-6 xl:w-6
          md:h-5 md:w-5
          2xs:h-4 2xs:w-4"
            fill="none"
        ...
        <div className="flex flex-col space-y-4 pt-2 items-left justify-left">
          <div className="flex mt-8 justify-center items-center">
            <img
              src={item.image}
              alt="Dish"
              className="w-[200px] h-auto rounded-3xl"
            />
          </div>

          <div className="flex flex-row mb-2 space-x-4 justify-center items-center mr-[2rem] ml-[2rem]">
            <h1 className="flex text-gray-700 text-2xl text-left font-bold">
              {item.title}
            </h1>
            <h5 className="flex text-gray-700 text-2xl text-left font-bold">
              {item.price}JOD
            </h5>
          </div>

          <div>
            <h5 className="block text-center text-gray-500 text-xs font-md mb-2 mr-[2rem] ml-[2rem]">
              {item.description}
            </h5>
          </div>
